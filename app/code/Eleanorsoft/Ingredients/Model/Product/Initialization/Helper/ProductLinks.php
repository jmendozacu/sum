...
namespace Eleanorsoft\Ingredients\Model\Product\Initialization\Helper;

use Eleanorsoft\Ingredients\Model\Catalog\Product\Link;
...
class ProductLinks
{
    ...
    {
        if(isset($links[self::TYPE_NAME]) && !$product->getIngredientsLinkData()) {

            $links = (isset($links[self::TYPE_NAME])) ? $links[self::TYPE_NAME] : $product->getIngredientsLinkData();
            if (!is_array($links)) {
                $links = [];
            }

            if ($product->getIngredientsLinkData()) {
                $links = array_merge($links, $product->getIngredientsLinkData());
            }
            $newLinks = [];
